...
count_graphs = [
    ...
    GroupByGraph(
        ...
        columns=[NumericColumn("dashed_length")],
        slider_value=1,
        full_grid_row=True,
        filter="(type = 'dashed') AND (dashed_length BETWEEN 0 AND 20)",
    ),
    GroupByGraph(
        title="Dashed Gap Length Distribution",
        columns=[NumericColumn("dashed_gap")],
        slider_value=1,
        full_grid_row=True,
        filter="(type = 'dashed') AND (dashed_gap BETWEEN 0 AND 25)",
    ),
    GroupByGraph(
        title="Lane Mark Width Distribution (m)",
        columns=[NumericColumn("avg_width")],
        slider_value=2,
        full_grid_row=True,
        filter="avg_width > 0 AND avg_width < 1.5",
    ),
    GroupByGraph(
    ...
    GroupByGraph(
        title="POC for array distribution",
        columns=[ArrayColumn("half_width", filter="BETWEEN 0 AND 25", unnest=True)],
        slider_value=0,
        full_grid_row=True,
        filter="type <> 'dashed'",
    ),
