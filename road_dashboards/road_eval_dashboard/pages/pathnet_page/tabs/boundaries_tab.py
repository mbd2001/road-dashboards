...
CATEGORICAL_CLASSES = [
    ("boundaries", "type"),
    ("boundaries", "is_re"),
    ("boundaries", "iscrossable"),
    ("road_edges", "iselevated"),
]


CLASS_NAMES_MAP = {
    "boundaries type": BOUNDARY_TYPES_NAMES,
    "boundaries iscrossable": ["True", "False"],
    "boundaries is_re": ["True", "False"],
    "road_edges iselevated": ["True", "False"],
}


def create_bound_name(bound, class_attribute):
    return f"{bound} {class_attribute}"


def parse_bound_name(bound_name):
    return bound_name.split(" ", 1)


EMPTY_FIGURE = {"data": [], "layout": {}}


def get_bound_column_templates(bound, class_attribute, gt_pred):
    if gt_pred == "gt" or gt_pred == "label":
        return f"{bound}_gt_{class_attribute}"
    elif gt_pred == "pred" or gt_pred == "prediction":
        return f"{bound}_{class_attribute}"
    else:
...
boundaries_layout = html.Div(
    ...
    + [
        html.Div(id={"type": "graph_output_div", "bound": create_bound_name(bound, attr)})
        for bound, attr in CATEGORICAL_CLASSES
    ]
)
...
def update_all_lr_store(nets, meta_data_filters, pathnet_filters, time_value, store_id):
    ...
    bound_name = store_id["bound"]  # e.g., "boundaries|type"
    ontology, class_attribute = parse_bound_name(bound_name)
    class_names = CLASS_NAMES_MAP[bound_name]

    lr_all_data = generate_matrices_graphs_lr(
        label_col_template=get_bound_column_templates(ontology, class_attribute, "gt"),
        pred_col_template=get_bound_column_templates(ontology, class_attribute, "pred"),
...
def generate_all_dps_figures(side, time_value, lr_all_store_data, graph_id):
    ...
    mat_title = f"Per-Class Accuracy for {side} {bound_name} ({role_str_acc}, t={time_value:.1f}s)"
...
def generate_host_figures(side, time_value, lr_host_store_data, graph_id):
    role = "host"
    role_str_acc = "Host dps"
    bound_name = graph_id["bound"]
    class_names = CLASS_NAMES_MAP[bound_name]

    if not lr_host_store_data:
        return EMPTY_FIGURE

    lr_all_data = deserialize_lr_data(lr_host_store_data)
    net_names = list(lr_all_data.get("all", {}).keys())
    if side not in lr_all_data:
        return EMPTY_FIGURE
...
def draw_conf_mat(net1_name, net2_name, side, time_value, all_lr_store_data, host_lr_store_data, graph_ids):
    ...
    conf_mat_title_all = f"Confusion Matrix for {side} {bound_name} ({role_str_conf_all}, t={time_value:.1f}s)"
    ...
    conf_mat_title_host = f"Confusion Matrix for {side} {bound_name} ({role_str_conf_host}, t={time_value:.1f}s)"
...
@callback(
    Output({"type": "graph_output_div", "bound": MATCH}, "children"),
    Input(NETS, "data"),
    State({"type": "graph_output_div", "bound": MATCH}, "id"),
)
def generate_boundaries_layout(nets, graph_id):
    """Generates the confusion matrix layout for a specific categorical class."""
    if not nets:
        return []

    bound_name = graph_id["bound"]

    all_dps_diag_compare_id = {"type": BOUNDARIES_ALL_DIAG_COMPARE, "bound": bound_name}
    host_diag_compare_id = {"type": BOUNDARIES_HOST_DIAG_COMPARE, "bound": bound_name}
    conf_mat_option_id_1 = {"type": "net_options", "bound": bound_name, "index": 1}
    conf_mat_option_id_2 = {"type": "net_options", "bound": bound_name, "index": 2}
    all_dps_conf_mat_id_1 = {"type": BOUNDARIES_ALL_CONF_MATS, "bound": bound_name, "index": 0}
    all_dps_conf_mat_id_2 = {"type": BOUNDARIES_ALL_CONF_MATS, "bound": bound_name, "index": 1}
    host_conf_mat_id_1 = {"type": BOUNDARIES_HOST_CONF_MATS, "bound": bound_name, "index": 0}
    host_conf_mat_id_2 = {"type": BOUNDARIES_HOST_CONF_MATS, "bound": bound_name, "index": 1}
    all_lr_store_id = {"type": BOUNDARIES_ALL_MATRICES_LR_STORE, "bound": bound_name}
    host_lr_store_id = {"type": BOUNDARIES_HOST_MATRICES_LR_STORE, "bound": bound_name}
        ...
        [
            dbc.Row(
                [
                    dbc.Col(
                        [
                            html.H6(f"All DP Per-Class Accuracy for {bound_name}", style={"textAlign": "center"}),
                    ...
                    dbc.Col(
                        [
                            html.H6(f"Host DP Per-Class Accuracy for {bound_name}", style={"textAlign": "center"}),
    ...
    controls_layout = card_wrapper(
        [
            dbc.Row([html.H4(f"Confusion Matrix Comparison for {bound_name}", style={"textAlign": "center"})]),
    ...
    confusion_matrices_layout = card_wrapper(
        [
            dbc.Row(
                html.H5(
                    f"All Data Points Confusion Matrices for {bound_name}",
            ...
            dbc.Row(
                html.H5(
                    f"Host Data Points Confusion Matrices for {bound_name}",
