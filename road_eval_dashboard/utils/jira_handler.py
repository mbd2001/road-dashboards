...
def get_jira_issues_from_prefix(prefix):
    ...
    if len(split_prefix) > 1 and split_prefix[0] == 'ROAD' and split_prefix[1] != "":
        ...
        for _ in range(num_to_multiply + 1, MAX_ISSUE_KEY_DIGITS):
            ...
            if or_condition != "":
                or_condition += " OR "
            or_condition += f'(issuekey >= "ROAD-{prefix_num*multiplier}" AND issuekey < "ROAD-{(prefix_num + 1)*multiplier}")'
        if or_condition == "":
            return []
        tickets = jira.search_issues(f'({or_condition}) AND project=ROAD ORDER BY issuekey', fields=["summary"])
    else:
        tickets = jira.search_issues(
            f'summary ~ "{prefix}" AND project=ROAD ORDER BY resolution, status, updated DESC',
            fields=["summary"])
    return tickets
