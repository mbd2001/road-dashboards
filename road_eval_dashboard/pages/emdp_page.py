...
def get_base_graph_layout(filter_name, sort_by_dist=False):
    layout = card_wrapper(
        [
            dbc.Row(loading_wrapper([dcc.Graph(id={
                                "out": "graph",
                                "filter": filter_name,
                                "emdp_type": EMDP_TYPE,
...
layout = html.Div(
    ...
    ] + [get_base_graph_layout(filter_name, sort_by_dist=filter_props.get('sort_by_dist', False)) for filter_name, filter_props in EMDP_FILTERS.items()]
)
...
def get_none_dist_graph(meta_data_filters, is_world, is_precision, filter_none_monotonic, nets, effective_samples, graph_id):
    if not nets:
...
def get_dist_graph(meta_data_filters, is_world, is_precision, filter_none_monotonic, by_dist, nets, effective_samples, graph_id):
    if not nets:
        return no_update, no_update
    filter_name = graph_id["filter"]
    filters = EMDP_FILTERS[filter_name]
...
def get_emdp_fig(meta_data_filters, nets, interesting_filters, effective_samples, is_world, is_precision, filter_none_monotonic, filter_name):
