...
def draw_meta_data_filters(
    data, interesting_columns, score_func, hover=False, effective_samples={}, title="", xaxis="Filter", yaxis="Fb Score", count_items_name="lane marks"
):
    ...
    for ind, row in data.iterrows():
        fig.add_trace(
            go.Scatter(
                ...
                name=row.net_id,
                hovertext=[f"{count_items_name}: " + str(row[f"count_{col}"]) for col in interesting_columns] if hover else None,
            )
        )
    fig.update_layout(
        title=f"<b>{title}<b>",
