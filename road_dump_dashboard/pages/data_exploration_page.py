...
def get_countries_heat_map(meta_data_filters, md_table):
    ...
    query = generate_count_query(md_table, meta_data_filters=meta_data_filters, group_by_column=group_by_column)
...
def get_dynamic_pie_chart(group_by_column, slider_value, meta_data_filters, md_table, meta_data_dict):
    if not md_table or not group_by_column:
        return no_update
